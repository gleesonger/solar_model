...
from . import data
from .models import DashboardContext, DashboardElements, DashboardState, SUMMARY_LATEST_KEYS
...
class RecentTab:
    ...
    def render_recent_tab(self) -> None:
        ...
        latest = {**live_power.values, "inverter_today": telemetry.latest["inverter_today"]}
        self.elements.live_table = render_live_today_table(day, latest)
    ...
    def refresh_recent_tab(self) -> None:
        ...
        if self.elements.live_table is not None:
            self.elements.live_table.rows = data.summary_rows(day, latest)
    ...
    def refresh_live_power(self) -> None:
        ...
        if table is None:
            return
        live_power = data.load_live_power(self.context.database_path)
        if (
    ...
    def _load_data(self) -> tuple[pd.DataFrame, data.TelemetryData, data.LivePowerData, pd.DataFrame, pd.DataFrame]:
        ...
        return (
            ...
            data.load_telemetry(self.context.database_path, self.context.timezone_name),
            data.load_live_power(self.context.database_path),
...
def render_live_today_table(dataframe: pd.DataFrame, latest: dict[str, float | None]) -> Table:
    ...
    return ui.table(columns=columns, rows=data.summary_rows(dataframe, latest), row_key="metric").props(
        "dense flat bordered"
